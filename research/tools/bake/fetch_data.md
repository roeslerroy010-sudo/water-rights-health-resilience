...
| HydroSHEDS / HydroBASINS / HydroRIVERS | Current formal route retains Asia HydroRIVERS for river flow-through/routing evidence. HydroBASINS standard levels and HydroSHEDS core `dir`/`acc` were downloaded historically as backup/cross-check inputs but are not consumed by the current bake path. | HydroRIVERS: <https://www.hydrosheds.org/products/hydrorivers>; HydroBASINS: <https://www.hydrosheds.org/products/hydrobasins>; core downloads via `data.hydrosheds.org`. | `raw/hydrosheds/` | Free for scientific/educational/commercial use under HydroSHEDS terms; downloading implies acceptance. | **CURRENTLY RETAINED:** `HydroRIVERS_v10_as_shp.zip` only. Backup `hybas_as_lev01-12_v1c.zip`, `hyd_as_dir_15s.zip`, and `hyd_as_acc_15s.zip` were removed from local raw storage on 2026-06-26 because the current formal route does not read them. |
